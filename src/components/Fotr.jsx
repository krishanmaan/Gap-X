import React from "react";

export default function Fotr() {
  return (
    <>
      <div
        className="footer-section-wrapper"
        style={{ boxSizing: "border-box", position: "relative" }}
      >
        <div
          className="footer-area section-padding bg-primary"
          style={{
            boxSizing: "border-box",
            padding: "15px 0px",
            paddingTop: "15px",
            paddingBottom: "15px",
            position: "relative",
           
          }}
        >
          <div
            className="container"
            style={{
              boxSizing: "border-box",
              width: "100%",
              paddingRight: "15px",
              paddingLeft: "15px",
              marginRight: "auto",
              marginLeft: "auto",
              maxWidth: "1140px",
              transition: "all 0.5s ease 0s",
              textDecoration: "none",
            }}
          >
            <div
              className="footer-section mt-50"
              style={{
                boxSizing: "border-box",
                borderTop: "1px solid rgb(13, 74, 157)",
                borderBottom: "1px solid rgb(13, 74, 157)",
                paddingTop: "50px",
                paddingBottom: "50px",
                marginTop: "50px",
              }}
            >
              <div
                className="row"
                style={{
                  boxSizing: "border-box",
                  display: "flex",
                  flexWrap: "wrap",
                  marginRight: "-15px",
                  marginLeft: "-15px",
                }}
              >
                <div
                  className="col-md-4 col-sm-8"
                  style={{
                    boxSizing: "border-box",
                    position: "relative",
                    width: "100%",
                    paddingRight: "15px",
                    paddingLeft: "15px",
                    flex: "0 0 33.3333%",
                    maxWidth: "33.3333%",
                  }}
                >
                  <div
                    className="footer-widget"
                    style={{ boxSizing: "border-box" }}
                  >
                    <div
                      className="footer-logo"
                      style={{ boxSizing: "border-box", marginBottom: "20px" }}
                    >
                      <a
                        href="https://html.electronthemes.com/"
                        style={{
                          boxSizing: "border-box",
                          color: "rgb(0, 123, 255)",
                          backgroundColor: "transparent",
                          transition: "all 0.5s ease 0s",
                          textDecoration: "none",
                        }}
                      >
                        <img
                          alt="cryptocoin image"
                          src="https://shiny-cendol-295c9e.netlify.app/assets/img/logo/lo.png"
                          style={{
                            boxSizing: "border-box",
                            borderStyle: "none",
                            verticalAlign: "middle",
                            maxWidth: "100%",
                          }}
                        />
                      </a>
                    </div>
                    <p
                      style={{
                        boxSizing: "border-box",
                        marginTop: "0px",
                        marginBottom: "10px",
                        fontFamily: '"Poppins", sans-serif',
                        color: "#bfdbff",
                        fontSize: "14px",
                        fontWeight: 300,
                        lineHeight: "27px",
                      }}
                    >
                      A new smart block chain based marketplace for trading
                      digital goods & assets according.
                    </p>
                    <div
                      className="social-links mt-20"
                      style={{ boxSizing: "border-box", marginTop: "20px" }}
                    >
                      <ul
                        className="d-flex"
                        style={{
                          boxSizing: "border-box",
                          margin: "0px",
                          padding: "0px",
                          listStyle: "none",
                          marginTop: "0px",
                          marginBottom: "0px",
                          display: "flex",
                        }}
                      >
                        <li
                          style={{
                            boxSizing: "border-box",
                            padding: "7px 0px",
                            borderRadius: "50%",
                            overflow: "hidden",
                            height: "37px",
                            width: "37px",
                            display: "flex",
                            justifyContent: "center",
                            alignItems: "center",
                            backgroundColor: "#072457",
                            marginLeft: "5px",
                            marginRight: "5px",
                            cursor: "pointer",
                          }}
                        >
                          <a
                            href="https://www.facebook.com/profile.php?id=61562116088413&sk=about"
                            style={{
                              boxSizing: "border-box",
                              backgroundColor: "transparent",
                              transition: "all 0.5s ease 0s",
                              color: "#ffffff",
                              fontSize: "14px",
                              fontWeight: 400,
                              cursor: "pointer",
                              textDecoration: "none",
                            }}
                          >
                            <i
                              className="fa fa-facebook"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                            <i
                              className="fa fa-facebook"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                          </a>
                        </li>
                        <li
                          style={{
                            boxSizing: "border-box",
                            padding: "7px 0px",
                            borderRadius: "50%",
                            overflow: "hidden",
                            height: "37px",
                            width: "37px",
                            display: "flex",
                            justifyContent: "center",
                            alignItems: "center",
                            backgroundColor: "#072457",
                            marginLeft: "5px",
                            marginRight: "5px",
                            cursor: "pointer",
                          }}
                        >
                          <a
                            href="https://x.com/GapXbit"
                            style={{
                              boxSizing: "border-box",
                              backgroundColor: "transparent",
                              transition: "all 0.5s ease 0s",
                              color: "#ffffff",
                              fontSize: "14px",
                              fontWeight: 400,
                              cursor: "pointer",
                              textDecoration: "none",
                            }}
                          >
                            <i
                              className="fa fa-twitter"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                            <i
                              className="fa fa-twitter"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                          </a>
                        </li>
                        <li
                          style={{
                            boxSizing: "border-box",
                            padding: "7px 0px",
                            borderRadius: "50%",
                            overflow: "hidden",
                            height: "37px",
                            width: "37px",
                            display: "flex",
                            justifyContent: "center",
                            alignItems: "center",
                            backgroundColor: "#072457",
                            marginLeft: "5px",
                            marginRight: "5px",
                            cursor: "pointer",
                          }}
                        >
                          <a
                            href="https://medium.com/@gapxbit"
                            style={{
                              boxSizing: "border-box",
                              backgroundColor: "transparent",
                              transition: "all 0.5s ease 0s",
                              color: "#ffffff",
                              fontSize: "14px",
                              fontWeight: 400,
                              cursor: "pointer",
                              textDecoration: "none",
                            }}
                          >
                            <i
                              className="fa fa-medium"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                            <i
                              className="fa fa-medium"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                          </a>
                        </li>
                        <li
                          style={{
                            boxSizing: "border-box",
                            padding: "7px 0px",
                            borderRadius: "50%",
                            overflow: "hidden",
                            height: "37px",
                            width: "37px",
                            display: "flex",
                            justifyContent: "center",
                            alignItems: "center",
                            backgroundColor: "#072457",
                            marginLeft: "5px",
                            marginRight: "5px",
                            cursor: "pointer",
                          }}
                        >
                          <a
                            href="https://www.quora.com/profile/GAP-X-BIT"
                            style={{
                              boxSizing: "border-box",
                              backgroundColor: "transparent",
                              transition: "all 0.5s ease 0s",
                              color: "#ffffff",
                              fontSize: "14px",
                              fontWeight: 400,
                              cursor: "pointer",
                              textDecoration: "none",
                            }}
                          >
                            <i
                              className="fa fa-quora"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                            <i
                              className="fa fa-quora"
                              style={{
                                boxSizing: "border-box",
                                fontVariant: "normal",
                                fontStyle: "normal",
                                fontKerning: "auto",
                                fontOpticalSizing: "auto",
                                fontFeatureSettings: "normal",
                                fontVariationSettings: "normal",
                                fontWeight: "normal",
                                fontStretch: "normal",
                                lineHeight: 1,
                                fontFamily: "FontAwesome",
                                fontSize: "inherit",
                                textRendering: "auto",
                                WebkitFontSmoothing: "antialiased",
                                transition: "all 0.3s ease 0s",
                                top: "8px",
                                display: "block",
                                position: "relative",
                                marginTop: "33px",
                              }}
                            />
                          </a>
                        </li>
                      </ul>
                    </div>
                  </div>
                </div>
                <div
                  className="col-md-2 col-sm-4 col-xs-6"
                  style={{
                    boxSizing: "border-box",
                    position: "relative",
                    width: "100%",
                    paddingRight: "15px",
                    paddingLeft: "15px",
                    flex: "0 0 16.6667%",
                    maxWidth: "16.6667%",
                  }}
                >
                  <div
                    className="footer-widget"
                    style={{ boxSizing: "border-box" }}
                  >
                    <h6
                      style={{
                        boxSizing: "border-box",
                        marginTop: "0px",
                        fontFamily: '"Poppins", sans-serif',
                        lineHeight: 1.5,
                        color: "#ffffff",
                        fontSize: "15px",
                        fontWeight: 600,
                        marginBottom: "30px",
                      }}
                    >
                      USEFULL LINKS
                    </h6>
                    <ul
                      style={{
                        boxSizing: "border-box",
                        margin: "0px",
                        padding: "0px",
                        listStyle: "none",
                        marginTop: "0px",
                        marginBottom: "0px",
                      }}
                    >
                      <li
                        style={{ boxSizing: "border-box", padding: "7px 0px" }}
                      >
                        <a
                          href="https://shiny-cendol-295c9e.netlify.app/navbar#"
                          style={{
                            boxSizing: "border-box",
                            backgroundColor: "transparent",
                            transition: "all 0.5s ease 0s",
                            color: "#bfdbff",
                            fontSize: "14px",
                            fontWeight: 400,
                            textDecoration: "none",
                          }}
                        >
                          Home
                        </a>
                      </li>
                      <li
                        style={{ boxSizing: "border-box", padding: "7px 0px" }}
                      >
                        <a
                          href="https://shiny-cendol-295c9e.netlify.app/navbar#aboutus"
                          style={{
                            boxSizing: "border-box",
                            backgroundColor: "transparent",
                            transition: "all 0.5s ease 0s",
                            color: "#bfdbff",
                            fontSize: "14px",
                            fontWeight: 400,
                            textDecoration: "none",
                          }}
                        >
                          ABOUT US
                        </a>
                      </li>
                      <li
                        style={{ boxSizing: "border-box", padding: "7px 0px" }}
                      >
                        <a
                          href="https://shiny-cendol-295c9e.netlify.app/navbar"
                          style={{
                            boxSizing: "border-box",
                            backgroundColor: "transparent",
                            transition: "all 0.5s ease 0s",
                            color: "#bfdbff",
                            fontSize: "14px",
                            fontWeight: 400,
                            textDecoration: "none",
                          }}
                        >
                          REFER & EARN
                        </a>
                      </li>
                      <li
                        style={{ boxSizing: "border-box", padding: "7px 0px" }}
                      >
                        <a
                          href="https://shiny-cendol-295c9e.netlify.app/navbar#"
                          style={{
                            boxSizing: "border-box",
                            backgroundColor: "transparent",
                            transition: "all 0.5s ease 0s",
                            color: "#bfdbff",
                            fontSize: "14px",
                            fontWeight: 400,
                            textDecoration: "none",
                          }}
                        >
                          CONTACT
                        </a>
                      </li>
                    </ul>
                  </div>
                </div>
                <div
                  className="col-md-2 col-sm-4 col-xs-6"
                  style={{
                    boxSizing: "border-box",
                    position: "relative",
                    width: "100%",
                    paddingRight: "15px",
                    paddingLeft: "15px",
                    flex: "0 0 16.6667%",
                    maxWidth: "16.6667%",
                  }}
                >
                  <div
                    className="footer-widget"
                    style={{ boxSizing: "border-box" }}
                  >
                    <h6
                      style={{
                        boxSizing: "border-box",
                        marginTop: "0px",
                        fontFamily: '"Poppins", sans-serif',
                        lineHeight: 1.5,
                        color: "#ffffff",
                        fontSize: "15px",
                        fontWeight: 600,
                        marginBottom: "30px",
                      }}
                    >
                      {"SOLUTIONS"}
                    </h6>
                    <ul
                      style={{
                        boxSizing: "border-box",
                        margin: "0px",
                        padding: "0px",
                        listStyle: "none",
                        marginTop: "0px",
                        marginBottom: "0px",
                      }}
                    >
                      <li
                        style={{ boxSizing: "border-box", padding: "7px 0px" }}
                      >
                        <a
                          href="https://shiny-cendol-295c9e.netlify.app/navbar#"
                          style={{
                            boxSizing: "border-box",
                            backgroundColor: "transparent",
                            transition: "all 0.5s ease 0s",
                            color: "#bfdbff",
                            fontSize: "14px",
                            fontWeight: 400,
                            textDecoration: "none",
                          }}
                        >
                          TOKENOMICS
                        </a>
                      </li>
                      <li
                        style={{ boxSizing: "border-box", padding: "7px 0px" }}
                      >
                        <a
                          href="https://shiny-cendol-295c9e.netlify.app/navbar#roadMap"
                          style={{
                            boxSizing: "border-box",
                            backgroundColor: "transparent",
                            transition: "all 0.5s ease 0s",
                            color: "#bfdbff",
                            fontSize: "14px",
                            fontWeight: 400,
                            textDecoration: "none",
                          }}
                        >
                          ROADMAP
                        </a>
                      </li>
                    </ul>
                  </div>
                </div>
                <div
                  className="col-md-4 col-sm-8"
                  style={{
                    boxSizing: "border-box",
                    position: "relative",
                    width: "100%",
                    paddingRight: "15px",
                    paddingLeft: "15px",
                    flex: "0 0 33.3333%",
                    maxWidth: "33.3333%",
                  }}
                >
                  <div
                    className="footer-widget"
                    style={{ boxSizing: "border-box" }}
                  >
                    <h6
                      style={{
                        boxSizing: "border-box",
                        marginTop: "0px",
                        fontFamily: '"Poppins", sans-serif',
                        lineHeight: 1.5,
                        color: "#ffffff",
                        fontSize: "15px",
                        fontWeight: 600,
                        marginBottom: "30px",
                      }}
                    >
                      Newsletter
                    </h6>
                    <p
                      style={{
                        boxSizing: "border-box",
                        marginTop: "0px",
                        marginBottom: "10px",
                        fontFamily: '"Poppins", sans-serif',
                        color: "#bfdbff",
                        fontSize: "14px",
                        fontWeight: 300,
                        lineHeight: "27px",
                      }}
                    >
                      Sign up to get our all latest news and{" "}
                    </p>
                    <form style={{ boxSizing: "border-box" }}>
                      <input
                        className="form-control radious-50 my-20"
                        type="email"
                        placeholder="Email"
                        style={{
                          boxSizing: "border-box",
                          margin: "0px",
                          fontFamily: "inherit",
                          overflow: "visible",
                          border: "1px solid rgb(206, 212, 218)",
                          transition:
                            "border-color 0.15s ease-in-out 0s, box-shadow 0.15s ease-in-out 0s",
                          display: "block",
                          width: "100%",
                          fontSize: "1rem",
                          fontWeight: 400,
                          lineHeight: 1.5,
                          backgroundClip: "padding-box",
                          borderColor: "rgba(0, 0, 0, 0)",
                          padding: "3px 20px",
                          color: "#ffffff",
                          backgroundColor: "#072457",
                          height: "46px",
                          backgroundPosition: "97% center",
                          backgroundRepeat: "no-repeat",
                          backgroundSize: "20px",
                          cursor: "auto",
                          outline: "none",
                          marginTop: "20px",
                          marginBottom: "20px",
                          borderRadius: "40px",
                          backgroundImage:
                            'url("data:image/png;base64,")',
                        }}
                      />
                      <a
                        className="btn gradient-btn rounded-50 w-100 hvr-shutter-out-horizontal"
                        href="https://shiny-cendol-295c9e.netlify.app/navbar"
                        style={{
                          boxSizing: "border-box",
                          textAlign: "center",
                          userSelect: "none",
                          background: "rgb(225, 225, 225)",
                          display: "inline-block",
                          verticalAlign: "middle",
                          transform: "perspective(1px) translateZ(0px)",
                          position: "relative",
                          backgroundColor: "rgb(225, 225, 225)",
                          transition: "all 0.5s ease 0s",
                          transitionDuration: "0.5s",
                          transitionProperty: "all",
                          padding: "10px 20px",
                          fontWeight: 400,
                          color: "#ffffff",
                          fontSize: "14px",
                          lineHeight: "26px",
                          borderRadius: "40px",
                          border: "1px solid #1c8dff",
                          backgroundImage:
                            "linear-gradient(to right, rgb(0, 198, 251) 0%, rgb(28, 141, 255) 0%, rgb(0, 91, 234) 100%)",
                          boxShadow: "rgba(9, 23, 82, 0.47) 0px 0px 95px",
                          cursor: "pointer",
                          textDecoration: "none",
                          width: "100%",
                        }}
                      >
                        SUBSCRIBE
                      </a>
                    </form>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      
    </>
  );
}
